...
import com.zitga.idle.quest.constant.QuestTag;
import com.zitga.idle.quest.service.QuestBuilderService;
...
/**
 * https://raw.githubusercontent.com/MorphiaOrg/morphia/r1.3.2/morphia/src/main/java/org/mongodb/morphia/mapping/DefaultCreator.java
 */
@BeanComponent
public class MorphiaObjectFactory implements ObjectFactory {

    private final QuestBuilderService questBuilderService;

    public MorphiaObjectFactory(QuestBuilderService questBuilderService) {
    ...
    public Map createMap(final MappedField mappedField) {
        return newInstance(mappedField != null ? mappedField.getCTor() : null, ConcurrentHashMap.class);
    }
    ...
    private <T> Class<T> getClass(final DBObject dbObj) {
        Class clazz = null;
        if (dbObj.containsField(QuestTag.QUEST_TYPE_TAG)) {
            int questType = (Integer) dbObj.get(QuestTag.QUEST_TYPE_TAG);
            clazz = questBuilderService.getQuestClass(questType);
        }
